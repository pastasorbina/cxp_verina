...
class Page extends MY_Controller {
	...
	function view_list( $c_status="Active", $pagelimit=10, $offset=0, $orderby='', $ascdesc='DESC', $encodedkey='' ) {
	...
	function add() {
		...
		if($this->form_validation->run() == FALSE ) {
			...
			if(!$this->db->insert($this->table_name)) {
				...
				//	$this->db->set('mr_foreign_id' , $insert_id );
		...
		if($this->form_validation->run() == FALSE ) {
			...
			if( !$this->db->update($this->table_name) ) {
				...
				foreach($mr_id_arr as $k=>$tmp) {
					...
					$this->db->set('mr_module' , $this->mod );
					if( $result ) {
						$this->db->where('mr_id' , $mr_id_arr[$k] );
	...
	function _set_db() {
		$this->db->set('b_code' , $this->b_code );
		...
		foreach($pos as $k=>$tmp) {
			$media[$pos[$k]] = $m_id[$k] ? $m_id[$k] : '0';
		}
		$media = serialize($media);
		$this->db->set('c_media' , $media);
	}
