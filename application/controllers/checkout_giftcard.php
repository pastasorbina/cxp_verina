...
class Checkout_giftcard extends MY_Controller {
	...
	function _load_topbar(){ $html = $this->sci->fetch('checkout_giftcard/topbar.htm'); 	$this->sci->assign('checkout_topbar' , $html);	}
	function index() { redirect(site_url().'checkout_giftcard/view_checkout'); }
	...
	function update_cart_qty() {
		...
		//	return false;
		//}
		$ok  = $this->icart->update_quantity_giftcard($cart_id, $new_qty);
		switch($ok) {
			case 'ok' :
				$ret['status'] = 'ok';
				$ret['msg'] = 'ok';
	...
	{
		if ($saldo > $this->userinfo['m_saldo'] ) {
	...
	function view_checkout() {
		...
		//set breadcrumbs
		$breadcrumb = array();
		$breadcrumb[] = "<a href='".site_url()."product/list_gift_card/' >Buy another Giftcard</a>";
		$breadcrumb[] = "Checkout";
		$this->sci->assign('breadcrumb' , $breadcrumb);
		...
		$this->form_validation->set_rules('unique', 'Unique', 'trim');
	...
	function verify_checkout() { 
		...
		//calculate total
		$grandtotal = $cart['cart_subtotal'] ;
		$total_payout = $grandtotal + $unique;
...
//----transaction start
		...
		$grandtotal = $cart_subtotal;
		$total_payout = $grandtotal + $unique;

   
		//start setting database for transaction
		$this->db->set('m_id' , $m_id); 
		$this->db->set('trans_type' , 'Giftcard');  

		$this->db->set('trans_pm_id' , 0);
		...
		foreach($cart_item as $k=>$tmp){
			...
			$this->db->set('transd_quantity' , $tmp['cg_quantity']);
			$this->db->set('transd_weight' , $tmp['p_weight']);
			$this->db->set('transd_price' , $tmp['cg_price']);
			$this->db->set('transd_subtotal' , $tmp['cg_subtotal']);
	...
	//----transaction complete
		...
		if($ok) {
			$ok = $this->icart->clear_giftcard();
			$ok = $this->send_checkout_email($insert_id);
			redirect(site_url()."checkout_giftcard/checkout_success/$insert_id");
		} else {
			redirect(site_url()."checkout_giftcard/checkout_failed");
		}
	}
	...
	function send_checkout_email($trans_id) {
		...
		$html = $this->sci->fetch('checkout_giftcard/email_checkout.htm');
